...
def classify_destination_with_explanation(
    ...
    # perform extraction using updated engine
    ...
    if status != "success":
        explanation["reason"] = (
            "Content extraction failed or returned no text; using fallback destination."
        )
        explanation["extraction_status"] = status
        if extraction.get("error"):
            explanation["error"] = extraction.get("error")
        return fallback_destination, explanation
    else:
        explanation["extraction_status"] = "success"
...
def show_move_explanation(file_name, destination, category, subject, explanation):
    ...
    status = explanation.get("extraction_status", "unknown").upper()
    print(f"Extraction status: {status}")
    if explanation.get("error"):
        print(f"Extraction error: {explanation.get('error')}")
    ...
    def process(self, file_path):
        ...
        if not skip_ai_classification:
            try:
                ...
                # semantic confidence has already been calculated inside
                # classify_destination_with_explanation and attached to
                # explanation.  Use that value instead of recomputing.
                semantic_conf = explanation.get("semantic_confidence")
        ...
        # determine what should happen based on the aggregated confidence score
        # (note that under the updated policy we no longer skip files purely
        # because of low confidence; routing is either auto or preview).
        ...
        if not initial_mode:
            if action_for_confidence == "auto":
                ...
                print("Routing to AUTO due to high confidence")
            elif action_for_confidence == "preview_warn":
                # low-confidence warning path
                print("Routing to PREVIEW due to low confidence")

        # Before making an action decision, compute safe destination and
        # detect duplicates.  Under the revised policy the only reasons we
        # will ever *automatically* skip a file are:
        #   1. safety guardrail violation (handled earlier)
        #   2. duplicate content at the destination
        # All other cases—especially low or medium confidence—are routed to
        # preview so that a human can intervene.  This ensures there is no
        # silent skip based on confidence.
        try:
            ...
            log_decision(
                file_path=file_path,
                action="error",
                reason=f"Failed to prepare destination: {e}",
                confidence=confidence,
                details={"error": str(e)},
            )
            if not initial_mode:
        ...
        try:
            ...
            if action_for_confidence == "auto":
            ...
            elif action_for_confidence == "preview":
                queue_for_preview = True
                warn = False
                ...
                # Fallback - be conservative and preview
                queue_for_preview = True
                warn = True

            if queue_for_preview:
                # Always include extraction status and warning flag
                ...
                # Annotate last queued entry with extra metadata
                if preview_mode.preview_queue:
                    preview_mode.preview_queue[-1]["extraction_status"] = (
                        explanation.get("extraction_status", "unknown")
                    )
                    preview_mode.preview_queue[-1]["warning"] = warn
                    preview_mode.preview_queue[-1]["explanation"] = explanation

                if not initial_mode:
                    msg = f"👁️  {file}: Added to preview queue"
                    if warn:
                        msg += " (low confidence warning)"
                    print(msg)
                ...
                log_decision(
                    ...
                    action="preview",
                    reason=(
                        "File queued for preview mode review"
                        if not warn
                        else "Low confidence - queued for preview with warning"
                    ),
                    category=category,
                    subject=memory_subject,
                    destination=safe_path,
